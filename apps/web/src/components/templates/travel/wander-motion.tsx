...
/**
 * WANDER - motion primitives (Anime.js v4 + Lenis).
 *
 ...
 * (`data-wa-reveal`), so no-JS sees a finished page; and everything animated
 * lives in a `createScope` reverted on unmount. The pulsing map pins and hover
 * zoom are CSS. All of it stops under reduced motion.
 */
...
export function RevealGroup({
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    for (const item of items) item.dataset.waReveal = 'pending';
    const scope = createScope({ root: el }).add(() => {
      animate(items, {
        opacity: [0, 1],
        translateY: [y, 0],
    ...
    return () => {
      scope.revert();
      for (const item of items) delete item.dataset.waReveal;
    };
  }, [step, delay, y]);
  return (
    <Tag ref={ref} className={className}>
...
export function LiftLines({ children, delay = 0, className, id, as: Tag = 'h2' }: RevealProps) {
  ...
  useIsomorphicLayoutEffect(() => {
    const el = ref.current;
    if (!el || prefersReducedMotion()) return undefined;
    const rows = Array.from(el.querySelectorAll<HTMLElement>('[data-wa-line]'));
...
export function Line({ children }: { children: ReactNode }) {
  return (
    <span className="wa-clip">
      <span data-wa-line className="block">
        {children}
      </span>
    </span>
  );
