...
/**
 * CRESTLINE - the template's motion primitives, built on anime.js v4.
 *
 ...
 *
 * The ambient loops - the drifting particle field, the rotating orbit ring - are
 * all CSS (see crestline.css), so this file only carries the scroll reveals and
 * the count-up. Keeping ambient loops in CSS means they cost nothing on the main
 * thread and stop cleanly under reduced motion.
 */
...
export function RevealGroup({
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    for (const item of items) item.dataset.crlReveal = 'pending';
    ...
    return () => {
      scope.revert();
      for (const item of items) delete item.dataset.crlReveal;
    };
  }, [step, delay, y]);
...
export function LiftLines({
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    const rows = Array.from(el.querySelectorAll<HTMLElement>('[data-crl-line]'));
...
export function Line({ children }: { children: ReactNode }) {
  return (
    <span className="crl-clip">
      <span data-crl-line className="block">
        {children}
      </span>
    </span>
  );
