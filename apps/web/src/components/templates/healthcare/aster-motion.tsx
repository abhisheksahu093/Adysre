...
/**
 * ASTER - motion primitives (Anime.js v4 + Lenis).
 *
 * Two rules match the other templates: the SERVER renders everything visible and
 * final, the staged state is applied client-side before paint (`data-as-reveal`),
 * so no-JS sees a finished page; and everything animated lives in a `createScope`
 * reverted on unmount. The soft heartbeat pulse is CSS. All of it stops under
 * reduced motion.
 */
...
export function RevealGroup({
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    for (const item of items) item.dataset.asReveal = 'pending';
    const scope = createScope({ root: el }).add(() => {
      animate(items, {
        opacity: [0, 1],
        translateY: [y, 0],
    ...
    return () => {
      scope.revert();
      for (const item of items) delete item.dataset.asReveal;
    };
  }, [step, delay, y]);
  return (
    <Tag ref={ref} className={className}>
...
export function LiftLines({ children, delay = 0, className, id, as: Tag = 'h2' }: RevealProps) {
  ...
  useIsomorphicLayoutEffect(() => {
    const el = ref.current;
    if (!el || prefersReducedMotion()) return undefined;
    const rows = Array.from(el.querySelectorAll<HTMLElement>('[data-as-line]'));
...
export function Line({ children }: { children: ReactNode }) {
  return (
    <span className="as-clip">
      <span data-as-line className="block">
        {children}
      </span>
    </span>
  );
