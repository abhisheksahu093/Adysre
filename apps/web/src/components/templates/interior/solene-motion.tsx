...
/**
 * SOLÈNE - motion primitives (Anime.js v4 + Lenis).
 *
 * Two rules match the other templates: (1) the SERVER renders everything visible
 * and final, the staged state is applied client-side before paint, so no-JS sees
 * a finished page; (2) everything animated lives in a `createScope`, reverted on
 * unmount. The signature here is a calm CLIP-PATH image reveal; ambient parallax
 * and hover zoom are CSS. All of it stops under reduced motion.
 */
...
/** Serif display that rises out of clipped rows. One `<Line>` per line. */
export function LiftLines({ children, delay = 0, className, id, as: Tag = 'h2' }: RevealProps) {
  const ref = useRef<HTMLElement>(null);
  useIsomorphicLayoutEffect(() => {
    const el = ref.current;
    if (!el || prefersReducedMotion()) return undefined;
    const rows = Array.from(el.querySelectorAll<HTMLElement>('[data-sol-line]'));
    if (rows.length === 0) return undefined;
    const scope = createScope({ root: el }).add(() => {
      animate(rows, {
        translateY: ['115%', '0%'],
        duration: 1100,
        delay: stagger(95, { start: delay * 1000 }),
        ease: 'out(4)',
        autoplay: onScroll({ enter: ENTER, repeat: false }),
      });
    });
...
export function Line({ children }: { children: ReactNode }) {
  return (
    <span className="sol-clip">
      <span data-sol-line className="block">
        {children}
      </span>
    </span>
  );
...
/**
 * The signature: a duotone plate unmasks from the bottom via clip-path as it
 * enters. The clip is applied client-side only, so the server (and no-JS) render
 * the image fully visible, and reduced motion skips it entirely.
 */
