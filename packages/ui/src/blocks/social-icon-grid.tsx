/**
 * Live preview for `social-icon-grid`. Mirrors the `typescript` variant.
 * Keep in step with `src/data/components/social.ts`.
 */
type SocialPlatform = 'x' | 'linkedin' | 'github' | 'instagram' | 'youtube' | 'facebook';
...
export function SocialIconGrid({ links = DEFAULT_LINKS, label = 'Find us online', className = '' }: SocialIconGridProps) {
  return (
...
export default function SocialIconGridPreview() {
  return <SocialIconGrid />;
}
